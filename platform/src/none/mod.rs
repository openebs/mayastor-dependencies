...
const NOPLATFORM_NS_VAR: &str = "NOPLATFORM_NS";
...
impl None {
    ...
    pub(super) fn new() -> Self {
        ...
        let platform_ns = std::env::var(NOPLATFORM_NS_VAR).unwrap_or(DUMMY_PLATFORM_NS.to_string());

        Self {
            platform_uuid: platform_uuid.trim().to_string(),
            platform_ns: PlatformNS::from(platform_ns),
        }
    }
}
